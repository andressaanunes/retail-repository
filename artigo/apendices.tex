\section{\esp Apêndice - Detalhamento Técnico e Código-Fonte}

Este apêndice descreve as tecnologias, linguagens e bibliotecas utilizadas no desenvolvimento da plataforma \textit{Retail Insights}. Abaixo, apresentamos as definições técnicas e o código-fonte integral para fins de documentação e reprodutibilidade.

\begin{description}
    \item[\textbf{Python 3.12:}] Linguagem de programação de alto nível, interpretada e de propósito geral. Utilizamos esta linguagem pela sua robustez no tratamento de dados e vasta disponibilidade de bibliotecas para IA. \url{https://www.python.org/}
    
    \item[\textbf{Streamlit:}] \textit{Framework} de código aberto em \textit{Python} para criação de aplicativos \textit{web} interativos. Facilita a visualização de dados de \textit{machine learning}. \url{https://streamlit.io/}

    \item[\textbf{Streamlit:}] \textit{Framework} de código aberto utilizado para a criação e o \textit{deploy} da aplicação, acessível em: \url{https://retail-expansion.streamlit.app/}.
    
    \item[\textbf{Pandas:}] Biblioteca voltada para manipulação e análise de dados. Oferece estruturas para manipular tabelas numéricas, essencial para o tratamento do arquivo \textit{.csv} deste projeto. \url{https://pandas.pydata.org/}
    
    \item[\textbf{Folium:}] Biblioteca que facilita a visualização de dados geoespaciais em mapas interativos, utilizando a biblioteca \textit{Leaflet.js}. \url{https://python-visualization.github.io/folium/}
    
    \item[\textbf{Gemini API (Google AI Studio):}] Interface de programação de aplicações (\textit{API}) que permite integrar os modelos de linguagem do Google para gerar os relatórios estratégicos. \url{https://aistudio.google.com/}

    \item[\textbf{Supabase:}] Plataforma de \textit{Backend-as-a-Service} (BaaS) baseada em código aberto. Utilizamos o \textit{Supabase} neste projeto para o armazenamento e gestão das respostas do formulário de experiência do usuário. \url{https://supabase.com/}
    
    \item[\textbf{Regex (Expressões Regulares):}] Método formal de busca e manipulação de cadeias de caracteres. Utilizamos para extrair coordenadas geográficas das respostas textuais da IA.
    
    \item[\textbf{CSV (Comma-Separated Values):}] Formato de arquivo para armazenamento de dados tabulares, onde cada registro é separado por vírgula.
\end{description}

\newpage

% Configuração do código-fonte
\begin{lstlisting}[language=Python, caption={Código-fonte integral da aplicação Starbucks Insights.}, label={ap:codigo_completo}]
